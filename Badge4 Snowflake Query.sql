...
UPDATE SWEATBAND_PRODUCT_LINE
SET PRODUCT_CODE ='SWT_LMG' where headband_description ='1 Lime Green Headband';
...
select TRIM($1) as PRODUCT_CODE,$2 as HEADBAND_DESCRIPTION ,$3 as WRISTBAND_DESCRIPTION
from @product_metadata/swt_product_line.txt
(file_format => zmd_file_format_2);
...
SELECT INITCAP(REPLACE(REPLACE(relative_path, '.png'), '_', ' ')) as product_name 
...
CREATE VIEW LOCATIONS.DENVER_BIKE_SHOPS AS
SELECT NAME
,  st_distance(
        st_makepoint('-104.97300245114094','39.76471253574085')
        ,coordinates
...
--arn:aws:iam::662615364654:user/4fe51000-s
...
create iceberg table identifier($table_name) (
  ...
  --Table CCT_OPB43148 successfully created.

select * from CCT_OPB43148;
